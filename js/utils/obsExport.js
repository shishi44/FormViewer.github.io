import { serializeConnectionForUrl } from "./tabular.js?v=30";
...
<html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>パワラジフォーマット OBS</title>
<style>
*{box-sizing:border-box}html,body{width:100%;height:100%;margin:0;background:transparent!important;overflow:hidden}body{display:grid;place-items:center;padding:12px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Noto Sans JP",sans-serif}.template-host{width:min(calc(100vw - 24px),820px)}.template-root{--name-font-size:${values.nameFontSize}px;--content-font-size:${values.contentFontSize}px;--content-height:${template.defaults.contentHeight}px;--content-line-height:${template.defaults.contentLineHeight}}.template-root[data-bold-text="true"] .response-name{font-weight:800!important}.template-root[data-bold-text="true"] .response-content{font-weight:700!important}.response-content{overflow-y:auto;overflow-x:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word}.response-name{overflow-wrap:anywhere}
${templateCss}
</style></head><body>
<div class="template-host"><article class="template-root" data-template=${templateIdJson} data-bold-text="${values.boldText ? "true" : "false"}"><header class="template-name-wrap"><p class="template-label" aria-hidden="true"></p><h1 class="response-name"></h1></header><div class="template-divider" aria-hidden="true"></div><div class="response-content"></div></article></div>
<script>
...
function scrollContent(delta){contentEl.scrollBy({top:delta,behavior:'smooth'})}
document.addEventListener('keydown',e=>{if(e.key==='ArrowLeft'){e.preventDefault();move(-1)}else if(e.key==='ArrowRight'){e.preventDefault();move(1)}else if(e.key==='ArrowUp'){e.preventDefault();scrollContent(-140)}else if(e.key==='ArrowDown'){e.preventDefault();scrollContent(140)}else if(e.key==='Home'){e.preventDefault();contentEl.scrollTo({top:0,behavior:'smooth'})}});
window.addEventListener('message',e=>{if(e.data?.type==='formviewer:navigate')move(Number(e.data.delta)||0)});render();
</script></body></html>`;

  const date = new Date().toISOString().slice(0, 10).replaceAll("-", "");
  downloadBlob(html, `powaraji-format-OBS_${date}.html`, "text/html;charset=utf-8");
}
