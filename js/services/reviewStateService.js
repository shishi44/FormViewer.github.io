...
export function connectionReviewKey(connection) {
  ...
  if (connection?.type === "csv") return `csv:${connection.fileName || "active"}`;
  return "none";
}

export function loadReviewedIds(connection) {
  const all = safeParse(storageGet(APP_CONFIG.reviewStateKey));
  const source = all[connectionReviewKey(connection)];
...
export function saveReviewedIds(connection, reviewedIds) {
  const all = safeParse(storageGet(APP_CONFIG.reviewStateKey));
  const key = connectionReviewKey(connection);
  all[key] = {
    reviewedIds: [...reviewedIds].map(String),
...
export function setReviewed(connection, reviewedIds, responseId, reviewed) {
  ...
  if (reviewed) next.add(String(responseId));
  else next.delete(String(responseId));
